...
#############
args.config_file = 'config_template_tcga_breast_major_two.yaml'
############

def infer_single_slide(model, features, label, reverse_label_dict, k=1):
    features = features.to(device)
    if features.dtype == torch.float16:
        features = features.to(torch.float32)#, label.to(torch.int32)
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    if data_args.process_list is None:
        ...
        df_f = pd.read_csv(data_args.process_list)['test'].dropna(axis=0)
        df['slide_id'] = df_f

        df_l = pd.read_csv(data_args.label_list)
        df_l['slide_id']=df_l['case_id']+'/'+df_l['slide_id']
        for i in range(len(df)):
            if df.loc[i, 'slide_id'] in df_l['slide_id'].to_list():
                label = (df_l.loc[df_l['slide_id']==df.loc[i, 'slide_id']])['label']
                df.loc[i]['label']=label.iloc[0]
    ...
    if model_args.feature_extractor_type == 'resnet50':
        ...
        preprocess = None
    elif model_args.feature_extractor_type == 'vit-b-16':
        import sys
        sys.path.append("/shared/j.jang/pathai/CLIP")
        import clip
        feature_extractor, preprocess = clip.load("ViT-B/16")
        feature_extractor = feature_extractor.visual
        #feature_extractor.to(torch.float32)
    else:
        ValueError("not implemented model")
    ...
    for i in range(len(process_stack)):
        ...
        #mask_file = os.path.join(r_slide_save_dir, slide_id.split('/')[1]+'_mask.pkl')
        mask_file = r_slide_save_dir+'_mask.pkl'
        ...
        block_map_save_path = r_slide_save_dir +'_blockmap.h5'
        #mask_path = os.path.join(r_slide_save_dir, '{}_mask.jpg'.format(slide_id))
        mask_path = r_slide_save_dir + '_mask.jpg'
        ...
        features_path = r_slide_save_dir+'.pt'
        #h5_path = os.path.join(r_slide_save_dir, slide_id+'.h5')
        h5_path = r_slide_save_dir + '.h5'
        ...
        if not os.path.isfile(h5_path) :
            _, _, wsi_object = compute_from_patches(wsi_object=wsi_object, 
                                            ...
                                            ref_scores=None, transform=preprocess)                
        ...
        if data_args.process_list is not None:
            process_stack.to_csv('{}.csv'.format(data_args.process_list.replace('.csv', '')+'process_stack'), index=False)
        else:
            process_stack.to_csv('{}.csv'.format(exp_args.save_exp_code)+'process_stack', index=False)
        ...
        for sample in samples:
            if sample['sample']:
                ...
                for idx, (s_coord, s_score) in enumerate(zip(sample_results['sampled_coords'], sample_results['sampled_scores'])):
                    os.makedirs(sample_save_dir+'/' + slide_id.split('/')[0], exist_ok=True)
                    ...
                    patch.save(os.path.join(sample_save_dir, '{}/{}_{}_x_{}_y_{}_a_{:.3f}.png'.format(slide_id.split('/')[0],idx, slide_id.split('/')[1], s_coord[0], s_coord[1], s_score)))
        ...
        heatmap_save_name = '_blockmap.tiff'
        #if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
        if os.path.isfile(r_slide_save_dir + heatmap_save_name):
            pass
        else:
            ...
            heatmap.save(r_slide_save_dir +'_blockmap.png')
            del heatmap

        #save_path = os.path.join(r_slide_save_dir, '{}_{}_roi_{}.h5'.format(slide_id, patch_args.overlap, heatmap_args.use_roi))
        save_path = os.path.join(r_slide_save_dir + '{}_roi_{}.h5'.format(patch_args.overlap, heatmap_args.use_roi))
        ...
        if heatmap_args.calc_heatmap:
            compute_from_patches(wsi_object=wsi_object, clam_pred=Y_hats[0], model=model, feature_extractor=feature_extractor, batch_size=exp_args.batch_size, **wsi_kwargs, 
                                attn_save_path=save_path, transform=preprocess, ref_scores=ref_scores)
        ...
        if not os.path.isfile(save_path):
            ...
            if heatmap_args.use_roi:
                ...
                save_path_full = r_slide_save_dir + '_roi_False.h5'
        ...
        #"""
        """
        heatmap_save_name = '{}_roi_{}_blur_{}_rs_{}_bc_{}_a_{}_l_{}_bi_{}_{}.{}'.format(float(patch_args.overlap),
                                                                                         int(heatmap_args.use_roi),
                                                                                         int(heatmap_args.blur),
                                                                                         int(heatmap_args.use_ref_scores),
                                                                                         int(heatmap_args.blank_canvas),
        ...
        if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
        #if os.path.isfile(p_slide_save_dir + heatmap_save_name):
            pass
            ...
            os.makedirs(os.path.join(p_slide_save_dir, slide_id.split('/')[0]), exist_ok=True)
