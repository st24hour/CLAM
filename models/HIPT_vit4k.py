...
# import HIPT_vit as vits
# from HIPT_vit import DINOHead
...
class VisionTransformer4K(nn.Module):
    ...
    def prepare_tokens(self, x):
        ...
        # print('1', x.size())                # [B, 256, 384]
        x = self.phi(x)
        # print('2', x.size())                # [B, 256, 192]
    ...
    def forward(self, x):
        ...
        # print('1.', x.size())           # [2, 257, 192]
        for blk in self.blocks:
            x = blk(x)
        # print('2.', x.size())           # [2, 257, 192]
        # x = self.norm(x)
        # print('1.', x.size())           # [2, 257, 192]               
        # print('2.', x[:, 0].size())     # [2, 192]       
        return x[:, 0]
...
def get_vit4k(pretrained_weights, arch='vit4k_xs'):
    ...
    model4k = globals()[arch](num_classes=0)
...
class ViT_4k_js_v1(nn.Module):
    def __init__(self,
                 model_4k_path: str,
                 freeze_4k = True):
        super(ViT_4k_js_v1, self).__init__()

        self.model4k = get_vit4k(model_4k_path) if model_4k_path else globals()['vit4k_xs'](num_classes=0)

    # @autocast()
    def forward(self, x):
        # print('1', x.size())            # [B, 256, 384]
        x = x.unfold(1, 16, 16).transpose(1,2)
        # print('2', x.size())            # [B, 384, 16, 16]
        feature_4k = self.model4k(x) # feature_4k: n*192
        # print(feature_4k.size())        # [B, 192]
        return feature_4k