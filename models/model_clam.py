...
class CLAM_SB(nn.Module):
    def __init__(self, attn = 'gated', size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384], "custom1":[512, 512, 256], "custom2":[768, 512, 256], "HIPT_feat": [192, 512, 256]}
...
class CLAM_MB(CLAM_SB):
    def __init__(self, attn = 'gated', size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384], "HIPT_4k_feat": [192, 512, 256], "HIPT_256_feat": [384, 512, 256]}
        size = self.size_dict[size_arg]
        fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
...
class CLAM_SB_multi(nn.Module):
    def __init__(self, attn = 'gated', size_arg = "small", dropout = False, k_sample=8, n_classes=2, n_classes_subtype=2,
                    instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, balance=[1,0,1]):
        ...
        self.attention_net = nn.Sequential(*fc)
        self.classifiers = nn.Linear(size[1], n_classes)
        self.classifiers2 = nn.Linear(size[1], n_classes_subtype)
        instance_classifiers = [nn.Linear(size[1], 2) for i in range(n_classes if balance[0] else n_classes_subtype)]
        self.instance_classifiers = nn.ModuleList(instance_classifiers)
        self.k_sample = k_sample
        self.instance_loss_fn = instance_loss_fn
        self.n_classes = n_classes
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        # print(M.size())     # [1, 512]
        logits = self.classifiers(M)
        sub_logits = self.classifiers2(M)
        if instance_eval:
            results_dict = {'instance_loss': total_inst_loss, 'inst_labels': np.array(all_targets), 
            'inst_preds': np.array(all_preds)}
        else:
        ...
        if return_features:
            results_dict.update({'features': M})
        return logits, sub_logits, A_raw, results_dict


class CLAM_MB_multi(CLAM_SB_multi):
    def __init__(self, attn = 'gated', size_arg = "small", dropout = False, k_sample=8, n_classes=2, n_classes_subtype=2,
                    instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, balance=[1,0,1]):
        nn.Module.__init__(self)
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384], "custom1":[512, 512, 256], "custom2":[768, 512, 256], 
        ...
        if balance[0]:
            ...
            self.classifiers = nn.ModuleList(bag_classifiers)
        if balance[1]:
            bag_classifiers2 = [nn.Linear(size[1], 1) for i in range(n_classes_subtype)] #use an indepdent linear layer to predict each class
            self.classifiers2 = nn.ModuleList(bag_classifiers2)
        if balance[2]:
            instance_classifiers = [nn.Linear(size[1], 2) for i in range(n_classes if balance[0] else n_classes_subtype)]
            self.instance_classifiers = nn.ModuleList(instance_classifiers)
        self.k_sample = k_sample
