...
def compute_from_patches(wsi_object, clam_pred=None, model=None, feature_extractor=None, batch_size=512,  
    attn_save_path=None, ref_scores=None, feat_save_path=None, transform=None, **wsi_kwargs):    
    top_left = wsi_kwargs['top_left']
    bot_right = wsi_kwargs['bot_right']
    patch_size = wsi_kwargs['patch_size']
    
    roi_dataset = Wsi_Region(wsi_object, t=transform, **wsi_kwargs)
    ...
    for idx, (roi, coords) in enumerate(roi_loader):
        roi = roi.to(device)
        #if roi.dtype == torch.float16:
        #    roi = roi.to(torch.float32)#, label.to(torch.int32)
        coords = coords.numpy()
        
        with torch.no_grad():
            roi = roi.to(feature_extractor.module.conv1.weight.dtype)
            features = feature_extractor(roi)

            if attn_save_path is not None:
                features = features.to(model.classifiers.weight.dtype)
