...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        img_med = cv2.medianBlur(img_hsv[:,:,1], mthresh)  # Apply median blurring #same to median filter
    ...
    def process_contours(self, patient_id, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        save_path_hdf5 = os.path.join(save_path, patient_id, str(self.name) + '.h5')
        print("Creating patches for: ", self.name, "...",)
        elapsed = time.time()
